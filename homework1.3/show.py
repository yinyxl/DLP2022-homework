...
class Show_authors_list(QWidget):
    def show1(self, author_dict):
        ...
        for key, value in author_dict.items():
            ...
            self.findButton.setGeometry(60, pos, 150, 40)
            ...
            self.findButton.setGeometry(240, pos, 250, 40)
    ...
    def get_articles(self, link_author):
        ...
        # 防止窗口闪退，需要成为全局变量
        self.table = Table()
        self.table.show2(article_title=article_title, sum_records=sum_records, year_published=year_published,\
                         link_article=link_article, venues=venues)
    ...
    def save_csv(self, link_author):
        ...
        with open(filepath, "a", newline='', encoding='utf-8') as file:
            ...
            for i in range(sum_records):
                info_sum = []
                info_sum.append(article_title[i])
                info_sum.append(year_published[i])
                info_sum.append(link_article[i])
                info_sum.append(venues[i])

                writer.writerow(info_sum)
            # filepath, type = QFileDialog.getSaveFileName(self, "save file", "the info of the scientist", 'csv(*.csv)')


class Table(QWidget):
    def show2(self, article_title, sum_records, year_published, link_article, venues):
        # super().__init__(article_title, sum_records, arg)
